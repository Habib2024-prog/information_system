...
import { createAmirObservation, createTeacherObservation } from "../../api/observations";
...
function isValidScore(value: string) {
  return /^(?:0|[1-2](?:\.\d{1,2})?|3(?:\.0{1,2})?)$/.test(value.trim());
}

export function ObservationFormDialog({ employee, kind, onOpenChange, onSaved }: ObservationFormDialogProps) {
  ...
  useEffect(() => {
    if (!open || !kind) return;
    setValues(emptyCommonValues());
    setScores(emptyScores(kind));
    setMemberSearch("");
    setValidationError("");
    setSubmissionError("");
  ...
  const title = kind === "teacher" ? "ثبت مشاهدهٔ معلم" : "ثبت مشاهدهٔ آمر / سرمعلم";
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (competencies.some(({ key }) => !isValidScore(scores[key] ?? ""))) {
      setValidationError("برای هر قابلیت، نمرهٔ معتبر از ۰ تا ۳ با حداکثر دو رقم اعشاری وارد کنید.");
      return;
    }
    setValidationError("");
    setSubmissionError("");
    ...
    try {
      if (kind === "teacher") {
        await createTeacherObservation(employee.id, {
          ...common,
        ...
        await createAmirObservation(employee.id, {
          ...common,
