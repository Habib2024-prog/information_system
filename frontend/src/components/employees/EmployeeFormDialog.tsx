...
export function EmployeeFormDialog({ open, onOpenChange, departments, employee, onSaved }: EmployeeFormDialogProps) {
  ...
  const fieldClass = (field: FieldName) => `motion-safe-transition h-10 w-full rounded-lg border bg-white px-3 text-sm text-ink focus:border-accent focus:ring-2 focus:ring-accent/15 ${errors[field] ? "border-rose-500" : "border-line"}`;
  const textAreaClass = (field: FieldName) => `motion-safe-transition min-h-24 w-full resize-y rounded-lg border bg-white px-3 py-2 text-sm text-ink focus:border-accent focus:ring-2 focus:ring-accent/15 ${errors[field] ? "border-rose-500" : "border-line"}`;
